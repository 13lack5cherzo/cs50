...
require 'src/Tile'
require 'src/TileMap'
require 'src/KeyLockObject'
require "src/FlagPost"
...
gSounds = {
    ...
    ["level-complete"] = love.audio.newSource('sounds/levelcomplete.wav', 'static')
}
...
gFrames = {
    ...
    ['keysLocks'] = GenerateQuads(gTextures['keysLocks'], TILE_SIZE, TILE_SIZE),  -- index from 1-8 (yellow is 1, 5)
    ['posts'] = GenerateQuads(gTextures['posts'], TILE_SIZE, 3*TILE_SIZE),  -- use index 1-6
    ['flags'] = GenerateQuads(gTextures['flags'], TILE_SIZE, TILE_SIZE)  -- use indices 7, 16, 25, 34
}
