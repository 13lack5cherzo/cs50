...
function TakeTurnState:init(battleState)
    ...
    self.expScalar = DEBUG_MODE and 5 or 1 -- experience scalar for debug
    ...
    Timer.after(0.5, function()
        ...
        :finish(function()
            ...
            :finish(function()
                ...
                if DEBUG_MODE and (attacker == self.playerPokemon) then  -- instantly kill opponent when debugging
...
function TakeTurnState:victory()
    ...
    :finish(function()
        ...
        function()
            ...
            exp = exp * self.expScalar  -- scale exp (for debug)
            ...
            Timer.after(1.5, function()
                ...
                :finish(function()
                    ...
                    if self.playerPokemon.currentExp > self.playerPokemon.expToLevel then
                        ...
                        -- get stat increases
                        self.statIncrease = {}
                        self.statIncrease["HP"], 
                        self.statIncrease["attack"], 
                        self.statIncrease["defense"], 
                        self.statIncrease["speed"] = self.playerPokemon:levelUp()

                        gStateStack:push(BattleMessageState('Congratulations! Level Up!',
                        function()
                            gStateStack:push(BattleLevelupState(self.battleState, self.statIncrease))
                        end))
                    else
                        self:fadeOutWhite()
                    end
