    ...
    void Start () {
        ...
        for (int z = 0; z < mazeSize; z++)
        {
            for (int x = 0; x < mazeSize; x++)
            {
                ...
                if (mapData[z, x])
                {
                    // fully tile to ceiling (including floor)
                    for (int y = 0; y < 4; y++)
                    {
                        CreateChildPrefab(wallPrefab, wallsParent, x, y, z);
                    }
                }
                // character has not been placed yet
                else if (!characterPlaced)
                {

                    // create floor for first spawn
                ...
                // character has been placed
                else
                {

                    // create floor with some probability (else leave a hole)
