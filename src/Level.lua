...
function Level:init()
    ...
    -- flag to indicate if anything has collided yet
...
function Level:update(dt)
    ...
    self.launchMarker:update(dt, self.hasCollided)
    ...
    if self.launchMarker.launched then
        ...
        if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 2) or love.keyboard.wasPressed('r') then
...
function Level:render()
    ...
    if not self.launchMarker.launched then
        ...
        love.graphics.setFont(gFonts['medium'])
        love.graphics.printf('space: powerup, R: reset',
            0, 64 + 16, VIRTUAL_WIDTH, 'center')
