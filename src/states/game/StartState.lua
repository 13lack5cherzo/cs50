...
function StartState:init()
    self.map = LevelMaker.generate(100, 10)  -- dummy map?
...
function StartState:update(dt)
    if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
        gStateMachine:change('play', {playerLevel = self.playerLevel, playerScore = self.playerScore})
    end
end
...
function StartState:render()
    ...
    love.graphics.setColor(0, 0, 0, 255)
    love.graphics.printf('level ' .. tostring(self.playerLevel), 1, VIRTUAL_HEIGHT / 2 - 5, VIRTUAL_WIDTH, 'center')
    love.graphics.setColor(255, 255, 255, 255)
    love.graphics.printf('level ' .. tostring(self.playerLevel), 0, VIRTUAL_HEIGHT / 2 - 6, VIRTUAL_WIDTH, 'center')
